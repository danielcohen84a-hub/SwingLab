...
def run_prediction():
    ...
    oracle = PlotlyOracle()
    html_report, png_report = oracle.generate_report(symbol, ticker_df, segments_df, prediction)
    
    # 7. THE ORACLE: Output
    print("\n" + "*"*60)
    print(f"  PREDICTION FOR {symbol}")
    print("*"*60)
    print(f"Predicted Swing Return: {prediction:.2%}")
    print(f"Interactive Report: {html_report}")
    
    if prediction > 0.04: msg = "STRONG BULLISH: Model detects high-velocity breakout."
    elif prediction > 0.01: msg = "MILD BULLISH: Positive drift expected."
    elif prediction < -0.04: msg = "STRONG BEARISH: Model warns of deep exhaustion."
    elif prediction < -0.01: msg = "MILD BEARISH: Expect some cooling off."
