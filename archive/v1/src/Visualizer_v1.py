...
class PlotlyOracle:
    ...
    def generate_report(self, ticker, ticker_df, segments_df, prediction_return):
        """
        Creates an interactive HTML dashboard and a static PNG report.
        """
        ...
        # Estimate Duration (Average of recent swings)
        avg_seconds = (segments_df['t_end'] - segments_df['t_start']).dt.total_seconds().median()
        target_time = last_time + timedelta(seconds=avg_seconds)
        ...
        fig.add_trace(go.Scatter(
            ...
            text=[None, f"<b>Target: {prediction_return:+.2%}</b>"],
