...
class SwingLabLSTM:
    ...
    def _build_model(self):
        ...
        # 6. Final Output: Predicts exactly 1 continuous number (the swing return slope).
    ...
    def save_weights(self, filepath='models/swinglab_lstm.weights.h5'):
        """Saves the trained neural network brain to the hard drive."""
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        self.model.save_weights(filepath)
        print(f"Model weights saved to {filepath}")

    def load_weights(self, filepath='models/swinglab_lstm.weights.h5'):
