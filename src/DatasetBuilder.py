...
class DatasetBuilder:
    ...
    def _create_sliding_windows(self, df, sequence_length):
        ...
        for symbol, group in grouped:
            # The targets for the dual-output model
            target_cols = ['swing_return', 'duration_hours_scaled']
            if not all(col in group.columns for col in target_cols):
                continue
            ...
            if not all(col in numerical_cols for col in target_cols):
                continue

            target_indices = [numerical_cols.index(col) for col in target_cols]

            # Identification of feature columns (everything EXCEPT targets)
            x_cols_indices = [idx for idx, col in enumerate(numerical_cols) if col not in target_cols]

            for i in range(len(data_matrix) - sequence_length):
                window_features = data_matrix[i: i + sequence_length, x_cols_indices]
                target = [data_matrix[i + sequence_length, idx] for idx in target_indices]
        ...
        final_feature_names = [col for col in numerical_cols if col not in target_cols]
